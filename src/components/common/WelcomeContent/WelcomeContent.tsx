...
export const WelcomeContent = ({
	...
	const { image, link } = about as AboutConfig
		...
		<div data-testid={testId} className={cn('block w-full xl:w-9/12', className)}>
			<div className={'flex flex-col lg:flex-row gap-4 lg:gap-5 justify-between'}>
				{!isParticipant && (
					<figure
						className={
							'relative w-36 md:w-40 lg:w-48 xl:w-52 h-36 md:h-40 lg:h-48 xl:h-52 flex-shrink-0'
						}
					>
						<Image className={'rounded-xl'} fill src={image} alt={t('imageAlt')} />
					</figure>
				)}
